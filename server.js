...
async function makeRestCall(url, method, body,key) {
...
async function callModel(message, settings, file1, history = []) {
    try {
        var openai = null;
        ...
        else if (settings.model == "o1-mini"){
            //set new environment variables
            process.env.OPENAI_BASE_URL = process.env.O1_MINI_BASE_URL2;
            settings.model = "o1-mini";
            openai = new AzureOpenAI({
                apiKey: process.env.O1_MINI_KEY2,
                baseURL: process.env.O1_MINI_ENDPOINT2,
                apiVersion: "2024-11-01-preview",
            });
 
        }
        else if(settings.model == "deepseek-0128-12"){
            // make a REST http request
            url = process.env.R1_ENDPOINT;
        }
        else if(settings.model == "ollama"){
            // make a REST http request
            console.log("setting ollama");
        ...
        console.log('Calling Azure OpenAI temp:', settings.temperature);
        ...
        if (settings.system_prompt) {
            ...
            {
                m1.unshift({ role: "user", content: settings.system_prompt });
            }
            else
            {
                m1.unshift({ role: "assistant", content: settings.system_prompt });
            }
        }
        if (file1) {
            //m1.push({ role: "user", content: message });
            //"image_url": {"url": f"data:image/jpeg;base64,{base64_image}"},
            m1.push({ role: "user", content: [{ type: "image_url", image_url: {"url": "data:image/jpeg;base64," + file1 }}] });
        } else {
            //m1.push({ role: "user", content: message });
        }
        ...
        if (settings.model == "o1-prpr-use2"){
            ...
            url = process.env.R1_ENDPOINT;
            var data1 = {"messages": m1, "max_tokens": settings.max_tokens, "temperature": settings.temperature, "stream": false, "model": settings.model};
            var completion = await makeRestCall(url, 'POST', data1, process.env.R1_KEY);
            console.log("finsihed call");
            console.log(completion);
        }
        else if(settings.model=="ollama"){
            url = process.env.OLLAMA_ENDPOINT;
            var data1 = {"messages": m1, "max_tokens": settings.max_tokens, "temperature": settings.temperature, "stream": false, "model": process.env.OLLAMA_MODEL};
            var completion = await makeRestCall(url, 'POST', data1, "");
            console.log("finsihed call");
            console.log(completion);
        }
        else if(settings.model=="o1-mini"){
...
io.on('connection', (socket) => {
    ...
    socket.on('chat message', async (data) => {
        ...
        const response = await callModel(data.message, data.settings, data.file, data.history);
        const userHistory = userHistories.get(socket.id);
        userHistory.push({ role: 'user', content: data.message });
        if (data.settings.model=="ollama")
        {
